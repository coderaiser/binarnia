...
module.exports = (options) => {
    ...
    for (const item of schema) {
        ...
        const parsedLength = parseLength(length, result);
        const resultOffset = itemOffset || currentOffset;
        
        const value = readHex(buffer, resultOffset, parsedLength);
...
function parseLength(length, result) {
    if (typeof length === 'number')
        return length;
    
    const name = length.replace(/[<>]/g, '');
